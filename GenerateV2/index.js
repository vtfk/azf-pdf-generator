...
const { decodeBase64, inspect } = require('@vtfk/utilities');
...
module.exports = async function (context, req) {
  ...
  try {
    ...
    let schemaDefaults = Sjablong.createObjectFromSchema(schema, true, req.body.preview);
    if(schemaDefaults) {
    ...
    let replaced = Sjablong.replacePlaceholders(template, combinedMetadata, req.body.preview);

    // Convert the template to HTML
    let markdownContent = Sjablong.getHTMLandMetadataFromMarkdown(replaced);
    ...
    let documentDefinition = undefined;
    ...
    if(documentDefinitionId) {
      ...
      if(!definition) {
        throw new HTTPError(404, `Could not find document definition '${documentDefinitionIdn}'`)
    ...
    // Generate the PDF
    let pdfBase64 = await PDFGenerator.GeneratePDFfromHTML(markdownContent.html, documentDefinition, documentStyles, combinedMetadata);
